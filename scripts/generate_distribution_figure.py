...
# Geometry. The y unit is one kRPS above Y_ORIGIN.
Y_ORIGIN = 960.0
FIRST_CENTER = 0.870
PAIR_PITCH = 1.300
...
def render_box(box: Box) -> list[str]:
    ...
    return [
        ...
        f"\\draw[whisker] ({center:.3f},{y(box.p5)}) -- ({center:.3f},{y(box.q1)});",
        f"\\draw[whisker] ({center:.3f},{y(box.q3)}) -- ({center:.3f},{y(box.p95)});",
        f"\\draw[whisker] ({cap_left:.3f},{y(box.p5)}) -- ({cap_right:.3f},{y(box.p5)});",
        f"\\draw[whisker] ({cap_left:.3f},{y(box.p95)}) -- ({cap_right:.3f},{y(box.p95)});",
        f"\\draw[{style}] ({left:.3f},{y(box.q1)}) rectangle ({right:.3f},{y(box.q3)});",
        f"\\draw[medianline] ({left:.3f},{y(box.median)}) -- ({right:.3f},{y(box.median)});",
    ]


def render(boxes: list[Box]) -> str:
    pairs = sorted({b.pair for b in boxes})
    ticks = ", ".join(f"{int(Y_ORIGIN) + n}/{n}" for n in (0, 20, 40, 60))
    labels = ", ".join(
