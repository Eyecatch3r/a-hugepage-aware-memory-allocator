...
TEMERAIRE_OFFLINE_SOURCES="${TEMERAIRE_OFFLINE_SOURCES:-0}"
...
BAZEL_OUTPUT_USER_ROOT="${BAZEL_OUTPUT_USER_ROOT:-${BUILD_DIR}/bazel-user-root}"
...
LLVM_BOOTSTRAP_CXXFLAGS="${LLVM_BOOTSTRAP_CXXFLAGS:-}"
...
sync_repo() {
  ...
  if [[ "${TEMERAIRE_OFFLINE_SOURCES}" == "1" ]]; then
    if [[ ! -d "${dir}" ]]; then
      echo "Offline source directory is missing: ${dir}" >&2
      exit 1
    fi

    local recorded_ref=""
    if [[ -f "${dir}/.temeraire-source-ref" ]]; then
      recorded_ref="$(cat "${dir}/.temeraire-source-ref")"
    elif [[ -d "${dir}/.git" ]]; then
      recorded_ref="$(git -C "${dir}" describe --tags --exact-match 2>/dev/null || git -C "${dir}" rev-parse HEAD)"
    fi

    if [[ "${recorded_ref}" != "${ref}" ]]; then
      echo "Offline source ref mismatch for ${dir}." >&2
      echo "Expected: ${ref}" >&2
      echo "Recorded: ${recorded_ref:-missing}" >&2
      exit 1
    fi

    echo "Using verified offline source: ${dir} (${recorded_ref})"
    return
  fi
...
if [[ "${BUILD_EXACT_LLVM}" == "1" ]]; then
  ...
  if [[ -n "${LLVM_BOOTSTRAP_CXXFLAGS}" ]]; then
    cmake_args+=( "-DCMAKE_CXX_FLAGS=${LLVM_BOOTSTRAP_CXXFLAGS}" )
  fi
...
if command -v bazelisk >/dev/null 2>&1; then
  ...
  curl -fsSL -o "${TOOLS_BIN}/bazelisk" https://github.com/bazelbuild/bazelisk/releases/download/v1.19.0/bazelisk-linux-amd64
  chmod +x "${TOOLS_BIN}/bazelisk"
  BAZEL_CMD="${TOOLS_BIN}/bazelisk"
fi
...
bazel_args=( --output_user_root="${BAZEL_OUTPUT_USER_ROOT}" build )
if [[ -n "${BAZEL_DISTDIR}" ]]; then
  bazel_args+=( --distdir="${BAZEL_DISTDIR}" )
fi
CC="${LLVM_CC}" CXX="${LLVM_CXX}" \
USE_BAZEL_VERSION="${USE_BAZEL_VERSION:-4.2.2}" "${BAZEL_CMD}" \
  "${bazel_args[@]}" \
...
if [[ "${BUILD_EXACT_LLVM}" == "1" ]]; then
  ...
  echo "llvm bootstrap cxxflags: ${LLVM_BOOTSTRAP_CXXFLAGS:-none}"
