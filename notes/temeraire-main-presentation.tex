...
\begin{tikzpicture}[node distance=0.62cm]
  \node[box, minimum width=2.9cm] (build) {Container\\bauen};
  \node[box, minimum width=3.0cm, right=0.62cm of build] (setup) {pinned\\Sources};
  \node[greenbox, minimum width=3.0cm, right=0.62cm of setup] (verify) {Preload\\pr\"ufen};

  \node[orangebox, minimum width=3.0cm, below=0.78cm of verify] (thp) {THP\\\smallcode{always}};
  \node[box, minimum width=3.0cm, left=0.62cm of thp] (run) {Paper-closer\\Run};
  \node[greenbox, minimum width=3.0cm, left=0.62cm of run] (aggregate) {Deltas\\aggregieren};

  \draw[arrow] (build) -- (setup);
  \draw[arrow] (setup) -- (verify);
  \draw[arrow] (verify) -- (thp);
  \draw[arrow] (thp) -- (run);
  \draw[arrow] (run) -- (aggregate);
\end{tikzpicture}
\end{center}
\vspace{0.45em}
\begin{itemize}
  \item \smallcode{run\_paper\_closer\_redis\_experiment.sh} ist der zentrale Pfad
  \item Balanced order wechselt zwischen Legacy-first und Temeraire-first
  \item \smallcode{manifest.txt} h\"alt Redis-Version, Trials, Requests und Reihenfolge fest
  \item pro Redis-Run entstehen \smallcode{summary.csv}, \smallcode{trials.csv}, THP- und Memory-Snapshots
\end{itemize}
\note{This slide ties the fixes into one repeatable command path.}
\end{frame}
...
\begin{frame}{Protokollbefund: Release-on}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.50\textwidth}
    \centering
    \small
  ...
  \end{column}
  \begin{column}{0.46\textwidth}
    \begin{alertblock}{Ausrei{\ss}er}
      Balanced 4 zeigte \pct{-12.02}, aber ohne Redis-Fehler und ohne
      offensichtlichen RSS- oder \smallcode{AnonHugePages}-Kollaps.
    \end{alertblock}
    \vspace{0.45em}
    \begin{block}{Diagnose-Rerun}
      Der gezielte Release-on-Rerun mit Temeraire zuerst kam auf \pct{+0.12}.
      Deshalb wird \pct{-12.02} nicht als stabiler Allocator-Effekt gelesen.
    \end{block}
    \vspace{0.35em}
    \scriptsize
  ...
  \end{column}
\end{columns}
\note{Do not hide the anomaly. The useful message is that the rerun made it look transient, not representative.}
\end{frame}

\begin{frame}{Einordnung der Befunde}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.49\textwidth}
    \begin{block}{St\"arkere Aussage}
      Release-off reproduziert die Richtung und die Gr\"o{\ss}enordnung des
      Redis-Signals: lokal \pct{+0.48} Median, Paper \pct{+0.75}.
    \end{block}
  \end{column}
  \begin{column}{0.47\textwidth}
    \begin{block}{Schw\"achere Aussage}
      Release-on bleibt nahe null bis klein positiv, mit einem gro{\ss}en
      lokalen Ausrei{\ss}er. Die exakte Table-1-Zahl wird nicht robust
      reproduziert.
    \end{block}
  \end{column}
\end{columns}
...
Das Artefakt reproduziert die Redis-Methode und landet oft im kleinen
Effektbereich des Papers, sobald THP wirklich aktiv ist.
\end{alertblock}
...
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.53\textwidth}
    ...
    \begin{itemize}
      ...
      \item Balanced Runs und gezielter Rerun zur Einordnung genutzt
    \end{itemize}
  \end{column}
  \begin{column}{0.43\textwidth}
    \begin{alertblock}{Endstand}
      Release-off zeigt ein wiederkehrendes kleines positives Signal.
      Release-on ist verrauscht. Das Ergebnis ist eine vorsichtige
      Redis-Reproduktion, keine Fleet-Reproduktion.
    \end{alertblock}
  \end{column}
\end{columns}
