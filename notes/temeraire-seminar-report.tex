...
advantage is $+0.48\%$, compared with the paper's $+0.75\%$. The attempted
release-on runs used an effective pageheap release rate of zero and are not a
measurement of the paper's release-on condition. The result should therefore
be read as a public reconstruction with a valid release-off comparison and an
open release-on question, not as a validation of the fleet-scale claims.
\end{abstract}
...
on  & $+0.44\%$ & not measured & corrected rerun required \\
\bottomrule
\end{tabular}
\end{table}

With THP active, the release-off comparison lands in the paper's small-effect
regime, but the run-to-run spread is larger than the median effect. Release-on
remains unmeasured.
...
There is no valid release-on result yet. Future runs must use an explicit
positive release rate and record the effective value. A rate sensitivity study
is preferable to choosing one value after observing its throughput. The reruns
should also collect \code{/proc/vmstat} hugepage counters, load average, and I/O
activity throughout the benchmark.
...
release-on experiment was misconfigured at a zero pageheap release rate and
must be repeated. The artifact reconstructs the public Redis path and documents
where that reconstruction failed. It does not validate Google's fleet-scale
claims or reproduce the exact production-era measurements.
