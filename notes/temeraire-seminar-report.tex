...
rather than across modes and rates.
\end{abstract}
...
should be stated plainly. The paper describes a trial of one million requests
``to push 5 elements and read those 5 elements''. The local trial issues one
million pushes and then one million reads, so two million commands in total. All
pushes precede all reads, which means \code{LRANGE} repeatedly reads the first
five elements of a list that has grown to five million elements rather than
reading each group just after it was pushed. An interleaved reading of the same
sentence, one push and one read per request, would produce a different access
pattern and a different live-heap shape. This report does not claim that its
...
Two further properties of this metric bound every comparison in
Section~\ref{sec:results}. First, it is raw request throughput as
\code{redis-benchmark} reports it, with no normalization for the CPU time
consumed to achieve it, so it does not separate a throughput gain from a change
in CPU cost. Second, it aggregates two operation rates that the paper never
reports separately. A local percentage delta and a paper percentage delta can
therefore sit close together while resting on differently constructed
quantities. Where this report calls a value close to the paper's, it means
numerical proximity between the local metric and the published number, not
agreement between two instances of the same measurement.
...
both are properties of the reconstruction rather than of the results. The local
metric is raw \code{redis-benchmark} throughput, aggregated across two operations
by a harmonic mean this report chose, and it carries no normalization for CPU
time; the published Redis deltas are single figures whose construction the
artifact cannot reproduce. Separately, the paper's one-million-request trial
``to push 5 elements and read those 5 elements'' admits more than one
implementation, and the sequential two-million-command form used here is one of
them. Both gaps mean a matching percentage is a weaker result than it looks.
...
explicit release configuration rather than across the experiment as a whole, and
it does so with a locally reconstructed workload and an unnormalized throughput
metric that the paper's figures do not share. Its main contribution is the path
from an unsuitable starting artifact, through build and release-control failures,
to a native run whose raw data and settings can be audited. Google's fleet-scale
claims and the production-era environment stay outside its reach.
