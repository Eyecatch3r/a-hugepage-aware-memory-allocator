...
The native run removes the extreme WSL2 swings. It gives a median Temeraire
delta of $-0.34\%$ with release disabled and $+0.55\%$ with release enabled at a
locally chosen 16~MiB/s, against the paper's $+0.75\%$ and $+0.44\%$, and
medians of $-0.60\%$ and $-0.08\%$ at 64 and 256~MiB/s. Reading the paper again
against the harness then exposed a metric mismatch, since the paper normalizes
throughput for CPU and the artifact did not record CPU time at all, so a
corrected series was run.

That series reproduces the paper's release-on result. Across four fresh pairs
the delta is $+0.401\%$ raw and $+0.474\%$ per CPU second, with all four pairs
positive and 95\% intervals of $[+0.20\%,+0.61\%]$ and $[+0.30\%,+0.65\%]$ that
exclude zero and contain the paper's $+0.44\%$. Release-off centres on zero and
stays unresolved at this sample size. Normalization proves to be worth under a
tenth of a percentage point here, because a single-threaded Redis under a
saturating benchmark holds one core throughout.
\end{abstract}
...
$[-1.04\%,+0.48\%]$, which does not reach the paper's $+0.75\%$. A second
four-pair sample in Section~\ref{sec:correction-results} centres on zero instead,
and its interval does contain that value, so this run should be read as one
sample of a condition that four pairs cannot resolve.
...
Two measurement gaps limited how far these comparisons could be pushed, and the
correction run in Section~\ref{sec:correction} measured the size of both. The
...
\section{Correction Run}
\label{sec:correction}

\subsection{Why the measurements were repeated}
\label{sec:why-rerun}

The results in Section~\ref{sec:results} were complete, audited, and wrong in
ways that only became visible afterwards. Three problems surfaced, in an order
worth recording, because the last two were faults in the harness that produced
those very numbers.
...
The second problem appeared eighteen hours into the first attempt at a corrected
run. A block died at trial 1875 of 2000 with \texttt{Connection reset by peer},
and the Redis log for that block showed a clean shutdown, which made the failure
look like a transient fault on a shared node. It was not. The run had been
started with a plain background \code{\&} from an interactive shell, so it stayed
inside the SSH session, and the session had ended. Redis ignores \code{SIGHUP}
at startup and \code{redis-benchmark} does not, so the hangup killed the client,
...
evidence against it. An hour of completed trials went in the bin for a signal
that had nothing to do with the allocator.
...
held the correct rates the whole time. A separate utility re-derives
\code{trials.csv} and \code{summary.csv} from them, so the sixteen hours were
repaired rather than repeated. The episode still points at a real weakness in
the audit: it compares the summary against the aggregated trial table, and both
descend from the same extraction, so it caught this only because
\code{KEYS[1]} is not a number. A plausible but wrong value would have passed.

\subsection{What the harness now records}

Four changes came out of the three problems. Each block now writes the CPU time
of the Redis server, taken from \texttt{redis-cli info cpu} at the end of the
block; since Redis restarts for every block, those counters are that block's
total rather than a running one. The workload is selectable, so both readings of
the paper's trial sentence can be measured instead of one being chosen and
defended. A failed benchmark invocation is retried up to four times, and every
attempt is written to a per-block log whose count reaches the audit as a warning,
so a retried block cannot be mistaken for a clean one. Runs start through a
launcher that puts them in their own session with \code{setsid} and ignores
\code{SIGHUP}, which removes the failure described above.

The rate extraction now drops the quoted test name before reading the field
that follows it, which also tolerates the extra latency columns that later
\code{redis-benchmark} versions emit.

\subsection{Results of the correction run}
\label{sec:correction-results}
...
\begin{table}[ht]
\caption{Correction run on node85, sequential workload. Deltas in raw
throughput and in requests per CPU second, the unit the paper reports. Intervals
are four-pair Student-t intervals on the log-transformed pair ratios and
accompany the mean.}
...
Release-on at 16~MiB/s is the result that changes. All four pairs favor
Temeraire and the mean is $+0.401\%$, with an interval that excludes zero.
Normalizing for CPU moves the mean to $+0.474\%$, and that interval excludes
zero as well. The paper's $+0.44\%$ falls inside both. This is the first
condition in the study that separates from the baseline in Temeraire's favor,
and it does so in the paper's own unit and within its reported value.

The improvement is in the variance, not the effect. The four pair deltas here
span $0.31$ percentage points, against $1.68$ for the corresponding pairs in
Table~\ref{tab:bare-metal-pairs}, and absolute throughput stayed between 982 and
995~kRPS across all sixteen blocks. Taking the two four-pair samples together,
seven of eight release-on pairs favor Temeraire.

Release-off tells a different story, and it corrects
Section~\ref{sec:results} rather than extending it. The mean is $+0.022\%$ and
the median $+0.037\%$, against a median of $-0.34\%$ in the first native run.
The two intervals overlap across almost their whole width, so the samples agree
with each other; what they agree on is that four pairs cannot resolve this
condition. Section~\ref{sec:results} read its negative aggregate as inconsistent
with the paper's $+0.75\%$ because the interval excluded that value. The
normalized interval here, $[-0.74\%,+0.93\%]$, contains it. That earlier reading
was too strong for the sample behind it, and this report withdraws it.
...
and combined blocks 14\,860 across 14\,900, so the Redis process held between
98 and 99.7 percent of a single core throughout. Redis~6.0.9 executes commands
on one thread and the benchmark keeps that thread saturated, which makes
requests per second and requests per second per core nearly the same quantity
for this workload. The metric gap identified in Section~\ref{sec:methodology} is
therefore real but small here, worth under a tenth of a percentage point. That
conclusion is specific to a single-threaded saturated server and should not be
carried over to the multithreaded applications in the paper's Table~1.
...
nothing usable. It shows that the choice left open by the paper's sentence is not
a detail: the two implementations differ by a factor of 3.6 in cost and by an
order of magnitude in the width of the interval they yield. The sequential form
is the better instrument, and the result in
