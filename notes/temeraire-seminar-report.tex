...
The artifact, the per-trial data behind every number here, and the source of
this report are public at
...
explorer, and the source of this document. A clone regenerates every reported
figure without a benchmark run, and \code{README.md} states the commands.
Section~\ref{sec:results} reports the numbers that those commands reproduce.
...
silently, and \code{LD\_PRELOAD} had leaked from the Redis server to
...
This section records the faults that changed a result or a method. The artifact
also carries a longer protocol note that logs every dead end in order, including
the ones that cost time without changing a number: the modern TCMalloc checkout
that would not wrap, the external Bazel workspace that inherited no Abseil
dependency, the target visibility that forced the wrapper into
\code{tcmalloc/testing}, and the LLVM bootstrap that needed
\texttt{-include cstdint} to build under a newer host compiler.

Four changes came out of the problems above. Each block now records the CPU time
of the Redis server from \texttt{redis-cli info cpu}, which is that block's total
...
be doing at the time, and the largest apparent Temeraire improvements turned out
to fall in the blocks that ran while the host was recovering from a slower
period. That correlation is what makes the numbers uninterpretable rather than
merely noisy: the allocator delta cannot be separated from the hour in which each
block happened to run. Its release-on median reached $+6.80\%$ at 64~MiB/s, one
256~MiB/s pair reached $+15.73\%$, and one release-on pair reached $-12.02\%$ and
did not reproduce when rerun on its own. All three are an order of magnitude
