...
One minor inconsistency remains worth recording: the paper-closer manifest still
marks \texttt{exact\_llvm\_commit\_cd442157cf=no}, even though the associated
system-information file confirms that the exact LLVM path was used in that run.
The methodology should therefore trust the collected compiler metadata over the
older manifest flag.
...
The intended final interpretation should therefore be cautious: release-off has
a recurring positive signal in the paper's direction, while release-on is small
and noisy. The artifact reproduces the Redis methodology and often lands in the
paper's small-effect regime, but it does not robustly reproduce the exact Redis
Table~1 statistics.
